...
// intended to refresh the schema definitions after a CRD has been added and is hopefully avaliable in k8s.
type refreshHandler struct {
	// debounceRef is the debouncableRefresher containing the Refreshable (typically the schema definition handler)
	debounceRef *debounce.DebouncableRefresher
	// debounceDuration is the duration that the handler should ask the DebouncableRefresher to wait before refreshing
...
// startBackgroundRefresh starts a force refresh that runs for every tick of durtation. Can be stopped
